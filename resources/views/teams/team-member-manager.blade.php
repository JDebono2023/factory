                ...
                <x-slot name="form">
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            <x-label for="role" value="{{ __('Role') }}" class="text-gray-900" />
                            <x-input-error for="role" class="mt-2" />

                            <div class="relative z-0 mt-1 border border-gray-200 rounded-lg cursor-pointer">
                                @foreach ($this->roles as $index => $role)
                                    <!-- Check if the user is the team owner or if the user belongs to the team -->
                                    @if (auth()->user()->ownsTeam($team) || ($team->id == 2 && $role->key == 'factory'))
                                        <button type="button"
                        ...
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('addTeamMember', $team) && Laravel\Jetstream\Jetstream::hasRoles() && $user->id !== auth()->user()->id)
                                        <button class="ml-2 text-sm text-gray-600 underline"
